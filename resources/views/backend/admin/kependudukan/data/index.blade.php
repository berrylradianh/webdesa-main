...
@section('content')
    <div class="p-6 bg-white rounded-xl shadow-md max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Data Kependudukan</h2>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 bg-white">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Perangkat
                            Desa</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($dataKependudukan as $item)
                        <tr class="hover:bg-gray-50 transition duration-200">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $item->tahun ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $item->user->username ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-semibold">
                                {{ number_format($item->jumlah_penduduk ?? 0) }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <button
                                    onclick="openModal({{ $item->id }}, '{{ $item->tahun }}', '{{ $item->user->username ?? '-' }}')"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="text-gray-500">
                                    <p class="text-lg font-medium mb-2">Belum ada data kependudukan.</p>
        ...
        {{-- Pagination --}}
        @if(method_exists($dataKependudukan, 'links'))
            <div class="mt-6">
    ...
    <div id="modal-dinamis" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Update Status Data Kependudukan</h3>
                <form action="" method="POST" id="form-dinamis">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status_select" id="status-select-dinamis" onchange="updateStatusHiddenDinamis()"
    ...
    {{-- JavaScript --}}
    <script>
        function openModal(id, tahun, perangkat) {
            ...
            form.action = '{{ url("admin/kependudukan") }}/' + id + '/update-status';
        ...
        document.getElementById('form-dinamis').addEventListener('submit', function (e) {
            const status = document.getElementById('status-dinamis').value;
            if (status === '') {
                e.preventDefault();
                alert('Silakan pilih status sebelum menyimpan!');
            }
        });
