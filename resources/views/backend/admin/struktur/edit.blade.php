...
@section('content')
    <div class="p-6 bg-white rounded-xl shadow-md max-w-2xl mx-auto">
        <h2 class="text-2xl font-bold mb-6 text-gray-700">Edit Jabatan</h2>
        ...
        <form action="{{ route('admin.positions.update', $position->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="type" class="w-full border rounded-lg px-3 py-2 focus:ring focus:ring-blue-200" required>
                    <option value="pemerintah" {{ $position->type == 'pemerintah' ? 'selected' : '' }}>Pemerintah</option>
                    <option value="bpd" {{ $position->type == 'bpd' ? 'selected' : '' }}>BPD</option>
                    <option value="lembaga" {{ $position->type == 'lembaga' ? 'selected' : '' }}>Lembaga</option>
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="parent_id" class="w-full border rounded-lg px-3 py-2 focus:ring focus:ring-blue-200">
                    ...
                    @foreach($parents as $parent)
                        <option value="{{ $parent->id }}" {{ $position->parent_id == $parent->id ? 'selected' : '' }}>
                            {{ $parent->name }}
                        </option>
                    @endforeach
                </select>
