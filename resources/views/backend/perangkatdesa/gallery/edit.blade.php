...
@section('content')
    <div class="p-6 bg-white rounded-xl shadow-md max-w-5xl mx-auto">
        ...
        <form action="{{ route(auth()->user()->role->id == 1 ? 'admin.gallery.update' : 'perangkat.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6 max-h-96 overflow-y-auto">
                    @foreach($gallery->items as $item)
                        ...
                        <div class="relative rounded-lg overflow-hidden shadow group">
                            ...
                            {{-- Form hapus --}}
                            <form action="{{ route(auth()->user()->role->id == 1 ? 'admin.gallery.item.destroy' : 'perangkat.gallery.item.destroy', $item->id) }}"
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route(auth()->user()->role->id == 1 ? 'admin.gallery.index' : 'perangkat.gallery.index') }}"
