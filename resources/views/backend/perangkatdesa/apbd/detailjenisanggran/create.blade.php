...
@section('page-title', 'Tambah Detail Jenis Anggaran')

@section('content')
<div class="max-w-5xl mx-auto bg-white p-8 rounded-2xl shadow-lg mt-8">
    <h2 class="text-3xl font-bold text-gray-800 mb-6 border-b-2 border-gray-200 pb-3">Tambah Detail Jenis Anggaran</h2>

    <form action="{{ route('detail_budget_types.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="group_budget_type_id" required
                    ...
                    <option value="{{ $group->id }}" {{ old('group_budget_type_id') == $group->id ? 'selected' : '' }}>{{ $group->name }}</option>
                    @endforeach
                </select>
