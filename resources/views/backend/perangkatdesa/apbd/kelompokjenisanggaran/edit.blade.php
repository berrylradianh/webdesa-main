...
@section('page-title', 'Edit Kelompok Jenis Anggaran')

@section('content')
<div class="max-w-5xl mx-auto bg-white p-8 rounded-2xl shadow-lg mt-8">
    <h2 class="text-3xl font-bold text-gray-800 mb-6 border-b-2 border-gray-200 pb-3">Edit Kelompok Jenis Anggaran</h2>

    <form action="{{ route('group_budget_types.update', $groupBudgetType->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Kelompok Jenis Anggaran</label>
            <input type="text" name="name" value="{{ old('name', $groupBudgetType->name) }}"
        ...
        <div>
            ...
            <input type="text" name="icon" value="{{ old('icon', $groupBudgetType->icon) }}"
                placeholder="Contoh: 📂 atau &lt;i class='fas fa-folder'&gt;&lt;/i&gt;"
