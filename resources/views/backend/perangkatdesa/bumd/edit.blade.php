...
@section('page-title', 'Edit Profil BUMD')

@section('content')
<div class="max-w-5xl mx-auto bg-white p-8 rounded-2xl shadow-lg mt-8">
    <h2 class="text-3xl font-bold text-gray-800 mb-6 border-b-2 border-gray-200 pb-3">Edit Profil BUMD</h2>

    <form action="{{ route('perangkat.bumd.update', $profile->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="type" value="{{ old('type', $profile->type) }}"
        ...
        <div>
            ...
            <textarea name="address" rows="4"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('address') border-red-500 @enderror transition duration-150">{{ old('address', $profile->address) }}</textarea>
            @error('address')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date" name="establishment_date" value="{{ old('establishment_date', $profile->establishment_date ? $profile->establishment_date->format('Y-m-d') : '') }}"
        ...
        <div>
            ...
            <textarea name="description" rows="5"
                class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror transition duration-150">{{ old('description', $profile->description) }}</textarea>
            @error('description')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="capital" value="{{ old('capital', $profile->capital) }}" step="0.01"
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="inactive" {{ old('status', $profile->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                </select>
                @error('status')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
